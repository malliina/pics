...
class TwitterValidator(redirCall: Call, conf: AuthConf, http: OkClient)
  ...
  def validateCallback(req: RequestHeader): Future[Result] = {
      ...
      fetchAccessToken(RequestToken(token), verifier).flatMap { maybeAccess =>
        maybeAccess.map { access =>
          fetchUser(access).mapR { user =>
            toResult(user.email.toRight(JsonError("Email missing.")))
          }.onFail { _ =>
            unauthorized
          }
        }.getOrElse {
    ...
    maybe.getOrElse(unauthorizedFut)
  }
  ...
  private def buildNonce =
    new String(Base64.getEncoder.encode(CognitoControl.randomState().getBytes(StandardCharsets.UTF_8)), StandardCharsets.UTF_8)
