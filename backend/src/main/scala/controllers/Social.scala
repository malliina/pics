...
import controllers.Social.SocialConf.loader
...
object Social {
  ...
  implicit val idLoader = loader(ClientId.apply)
  implicit val secretLoader = loader(ClientSecret.apply)
  ...
  object SocialConf {
    ...
    def loader[T](build: String => T) = ConfigLoader.stringLoader.map(s => build(s))

    def readCredentials(conf: Configuration) =
      AuthConf(conf.get[ClientId]("id"), conf.get[ClientSecret]("secret"))
  }
...
//    oauthConf(reverse.appleCallback(), conf.apple),
    ???,
    AppleTokenValidator(Seq(conf.apple.clientId))
  )

  private val amazonOauth =
    ???
//  OAuthConf(reverse.amazonCallback(), cognitoHandler, conf.amazonConf, http)
