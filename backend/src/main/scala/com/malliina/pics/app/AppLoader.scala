...
import com.malliina.pics.db.{DatabaseConf, DoobieDatabase, DoobiePicsDatabase}
...
object LocalConf {
  ...
  val localConf = Configuration(localConfig)
}
...
abstract class AppComponents(context: Context)
  extends BaseComponents(
    context,
    c => DatabaseConf.fromConf(c).fold(err => throw new Exception(err), c => AppConf(c))
  ) {
  ...
  override def buildPics() = MultiSizeHandler.default()
}
...
object AppConf {
  def apply(conf: DatabaseConf) = new AppConf {
    override def database: DatabaseConf = conf
...
//  val cache = new Cached(defaultCacheApi)
  ...
  val pics = new PicsController(html, service, sockets, auth, controllerComponents)
